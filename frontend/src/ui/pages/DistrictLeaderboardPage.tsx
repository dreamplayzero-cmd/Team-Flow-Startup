...
export const DistrictLeaderboardPage: React.FC<DistrictLeaderboardPageProps> = ({ onBack, onShowReport }) => {
    ...
    const sortedResults = results.map(report => ({
        name: report.area_name,
        score: parseFloat(report.final_score.toFixed(1)),
        prob: Math.round(report.success_prob * 100),
        id: report.area_name
    })).sort((a, b) => b.score - a.score);

    // Default placeholders if results are empty
    const rank1 = sortedResults[0] || { name: '성수동', score: 85.9, prob: 92, id: 'seongsu' };
    const rank2 = sortedResults[1] || { name: '한남동', score: 78.4, prob: 76, id: 'hannam' };
    const rank3 = sortedResults[2] || { name: '반포동', score: 72.1, prob: 64, id: 'banpo' };
                ...
                <main className="pt-28 pb-24 px-10 md:px-16 max-w-7xl mx-auto">
                        ...
                        >
                            ...
                            <div className="relative z-10">
                                ...
                                <p className="text-base text-white/70 leading-relaxed font-medium">
                                    현재 분석 데이터 기준 <strong className="text-stitch-secondary-fixed font-black">{rank1.name}</strong> 지역이 가장 높은 사업 타당성을 보이고 있습니다.
                                </p>
                            </div>
                        </motion.div>
                    </header>
                    ...
                    <section className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-20">
                        ...
                        >
                            ...
                            <div className="flex justify-between items-start mb-16 z-10">
                                <span className="bg-stitch-primary text-white px-6 py-2 rounded-full text-[12px] font-black tracking-[0.2em] uppercase">RANK #2</span>
                            </div>
                            <h2 className="text-5xl font-st-headline font-black text-stitch-primary mb-2 z-10 tracking-tight">{rank2.name}</h2>
                            <p className="text-stitch-on-surface-variant/40 text-[10px] mb-12 z-10 font-black uppercase tracking-widest">유동성 활기도 2위</p>
                            <div className="mb-12 z-10">
                        ...
                        >
                            ...
                            <div className="flex justify-between items-start mb-16 z-10">
                                <span className="bg-stitch-primary text-white px-6 py-2 rounded-full text-[12px] font-black tracking-[0.2em] uppercase">RANK #3</span>
                            </div>
                            <h2 className="text-5xl font-st-headline font-black text-stitch-primary mb-2 z-10 tracking-tight">{rank3.name}</h2>
                            <p className="text-stitch-on-surface-variant/40 text-[10px] mb-12 z-10 font-black uppercase tracking-widest">경쟁 강도 안정 지역</p>
                            <div className="mb-12 z-10">
