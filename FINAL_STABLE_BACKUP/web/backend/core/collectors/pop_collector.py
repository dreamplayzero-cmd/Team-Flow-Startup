...
class PopulationCollector:
    ...
    def collect_by_area(self, area_code, year_month=None):
        ...
        :param year_month: 통계년월 (YYYYMM) — 미지정 시 자동 계산
        """
        if not year_month:
            # WHY: 미래 날짜 API 500 에러, 확정 통계 발표는 통상 1~2개월 지연 → 현재 시점 기준 2개월 전 자동 계산
            year_month = self._get_latest_year_month()
        logger.info(f"[INFO] [pop_collector] [collect_by_area] - 대상 연월: {year_month}")
    ...
    @staticmethod
    def _get_latest_year_month():
        """
        현재 시점 기준 최신 확정 통계 연월 자동 계산
        WHY: 행안부 인구통계는 1~2개월 지연 발표됨.
             하드코딩된 '202401'대신 현재 2개월 전을 동적으로 계산하면
             항상 최신 데이터를 가져올 수 있음.
        """
        target_date = datetime.now() - timedelta(days=60)  # 2개월 전
    ...
    def _generate_fallback_data(self, area_code, year_month):
        ...
        # WHY: 랜덤 대신 결정론적 Mock 사용 — 지역별 일관된 값 보장
        import hashlib
        h = int(hashlib.md5(area_code.encode()).hexdigest(), 16)
        mz_pop = 10000 + (h % 45000)  # 10,000 ~ 55,000 범위 (deterministic)
        ...
        VALUES (?, ?, ?, ?, ?, ?)
        """
        data = (area_code, "MZ", mz_pop, f"{year_month[:4]}-{year_month[4:]}-01", 1, "LOW")

        try:
            self.db.execute_query(insert_query, data)
            logger.warning(
                f"[WARNING] [pop_collector] [_generate_fallback_data] - "
                f"인구 Fallback 저장 (is_mock=1): {area_code} = {mz_pop}명"
            )
            return True
        except Exception as e:
    ...
    def _save_to_db(self, rows, area_code, year_month):
        ...
        if processed_data:
            # WHY: 실제 API 성공 데이터는 is_mock=0, data_quality='HIGH'로 저장
